...
2. **Sequence.** In `outputs/audit-progress.md`, Stage 8 (Continuity / Room-Myopia Audit) must be `Complete` or `Skipped`. If it isn't, refuse, point the user to `/claim-continuity-audit`, and stop.

Proceed only when both pass.

Note: scope has grown materially across the prior stages — recompute storage volume and debris tonnage; the carrier's original numbers are almost certainly stale.
