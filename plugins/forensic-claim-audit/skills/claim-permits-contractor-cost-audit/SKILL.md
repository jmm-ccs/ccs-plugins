...
2. **Sequence.** In `outputs/audit-progress.md`, Stage 11 (Trades Audit) must be `Complete` or `Skipped`. If it isn't, refuse, point the user to `/claim-trades-audit`, and stop.

Proceed only when both pass.

Note: the Trades Audit (Stage 11) produced a reconciled trade list that drives this stage — read it from `outputs/stage-outputs/` before starting.
