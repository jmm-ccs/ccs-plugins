...
2. **Where it came from** — the exact basis that proves the point. It must be **one** of these three, and you must say which it is:
   - **A named project file** — the actual file name and the specific location inside it (carrier PDF page / item #, photo file name, sketch or Matterport area, drying-log date, prior agreed suggestion #), plus what that source shows. Name the real file, never a category or internal bucket name (§9).
