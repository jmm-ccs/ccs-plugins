...
| Sample supplement | Optional (legacy) | An example supplement document, labeled as a sample. It was the format template for the legacy Word supplement package (`claim-supplement-package`), now superseded — the standard deliverable is the marked-up copy of the carrier's estimate, which doesn't use it. Only needed if the project specifically wants the legacy Word supplement; standard final delivery no longer asks for it. |
