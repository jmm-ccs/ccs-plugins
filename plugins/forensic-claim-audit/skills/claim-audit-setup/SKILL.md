...
4. **Create the live suggestion-list artifact** (id `claim-audit-suggestion-list`) if it doesn't already exist. Follow the §2.3 Initialization sequence: read the template at `forensic-claim-audit/assets/suggestion-list-artifact.html`, replace the two `{{PROJECT_NAME}}` placeholders with the workspace folder's name, embed the current suggestion-list rows as JSON in `<script id="suggestion-list-data">`, write to `outputs/audit-suggestion-list-artifact.html`, and call `mcp__cowork__create_artifact`.

5. **Create the live audit-progress artifact** (id `claim-audit-progress`) if it doesn't already exist. Follow the §2.6 Initialization sequence: read the template at `forensic-claim-audit/assets/audit-progress-artifact.html`, replace the two `{{PROJECT_NAME}}` placeholders with the workspace folder's name. Write to `outputs/audit-progress-artifact.html`, and call `mcp__cowork__create_artifact`.

6. **Create the consolidated audit-findings artifact** (id `claim-audit-findings`) if it doesn't already exist (§2.9). Read the template at `forensic-claim-audit/assets/audit-findings-artifact.html`, replace the `{{PROJECT_NAME}}` placeholder with the workspace folder's name (the embedded `findings-data` starts empty), write to `outputs/audit-findings-artifact.html`, and call `mcp__cowork__create_artifact`. This single artifact is where every stage records its findings.
