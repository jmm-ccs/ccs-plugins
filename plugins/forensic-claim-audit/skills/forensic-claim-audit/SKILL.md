...
2. Use `AskUserQuestion` to ask whether to switch to single-session for this run. The question must give the user enough to actually decide — what each mode buys them, in plain consequence terms:
   ...
   - All 13 stages here in one chat: faster end-to-end, everything in one place. Cost: one long chat to scroll/return to.
...
4. **Create the live suggestion-list artifact** (id `claim-audit-suggestion-list`) if it doesn't already exist. Read `forensic-claim-audit/assets/suggestion-list-artifact.html`, replace the two `{{PROJECT_NAME}}` placeholders with the workspace folder's name (e.g., `Greensboro Claim`), embed the current suggestion-list rows as JSON in the `<script id="suggestion-list-data">` block, write to `outputs/audit-suggestion-list-artifact.html`, and call `mcp__cowork__create_artifact`.

5. **Create the live audit-progress artifact** (id `claim-audit-progress`) if it doesn't already exist. Read `forensic-claim-audit/assets/audit-progress-artifact.html`, replace the two `{{PROJECT_NAME}}` placeholders with the workspace folder's name. The template already has all 14 stages embedded at status `Not started`, so no JSON edit is needed for a fresh audit. Write to `outputs/audit-progress-artifact.html`, and call `mcp__cowork__create_artifact`.

5b. **Create the consolidated audit-findings artifact** (id `claim-audit-findings`) if it doesn't already exist (§2.9). Read `forensic-claim-audit/assets/audit-findings-artifact.html`, replace the `{{PROJECT_NAME}}` placeholder with the workspace folder's name (the embedded `findings-data` starts empty), write to `outputs/audit-findings-artifact.html`, and call `mcp__cowork__create_artifact`. Every stage records its findings into this one artifact.

6. **Divide the property into macro-areas** (§2.8 of the protocols). Read whatever project docs exist — sketch, Matterport, photos, walkthrough-video frames and transcript (`video-intake/`), the carrier estimate — and propose a division of the property into macro-areas (large physical sections grouping rooms/categories). Show it, ask the user to confirm or adjust, then write `outputs/macro-areas.md` per the §2.8 structure with `**Last updated:** at project setup`. If no project docs are present, ask the user to name the macro-areas, or note the map will be set during the Scope Audit. Stage 1 will reconcile this map once the true scope is confirmed.
...
The Stage 13 gate (handled by the loop in "How to walk the stages" → §4 of the protocols → single-session branch) will have already issued the *"Ready for Output Process."* prompt. When the user replies "begin output process" (or equivalent), use `Read` on `../claim-audit-finalizer/SKILL.md` and execute it. The finalizer runs the Supplement Sanity Audit, flags items that might trigger insurance-fight or homeowner-negotiation friction (asks the user about each), exports the suggestion list to XLSX, and invokes `claim-pdf-annotator` to produce the annotated carrier PDF. Both deliverables land in `outputs/` in the project folder. CCS uses those deliverables alongside Xactimate to build the actual carrier-facing supplement — Claude does not produce that supplement.
